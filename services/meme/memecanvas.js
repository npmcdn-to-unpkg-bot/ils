...
var drawText = function(context, pos, text, width, height, styleObj) {
    var fontSize = 100;
    context.textAlign = "center";
    context.fillStyle = styleObj.fillStyle
    context.strokeStyle =styleObj.strokeStyle //"#000";
    context.lineWidth = styleObj.lineWidth//6;

    while(1) {
        context.font = "bold " + fontSize + "px "+styleObj.fontFamily;
...
memecanvas.generate = function(file, topText, bottomText, styleObj, next){
    image.get(file, function(img){
        if(img){
            try{
                var ctx = image.ctx(img.canvas.width, img.canvas.height);
                ctx.drawImage(img.canvas, 0, 0);
                ctx = drawText(ctx, 'top', topText, img.canvas.width, img.canvas.height, styleObj);
                ctx = drawText(ctx, 'bottom', bottomText, img.canvas.width, img.canvas.height, styleObj);
