...
export default class App extends Component {
    ...
    componentDidMount() {
        console.log(singleHeight, singleWidth, fontSizeIs)
        emitter.on("init", ()=>{
            let willCheck = []
            let willBeVisible = []
            let willBeHidden = []
        ...
        emitter.on("add word tip", ()=>{
            if (!this.state.recentlyTipped) {
                let local = this.state.willBeVisible
                local[ this.state.index ] = `${this.state.currentCheck} `
                let stateBefore = R.join(" ", R.init(R.split(" ", this.state.inputFieldValue)))
                this.setState({
                    ...
                    currentCheck: this.state.willCheck[ this.state.index + 1 ],
                    index: this.state.index + 1,
                    previousWordStyle: "tag is-danger",
                    currentWordStyle: "tag is-warning",
                    wordIndex: 0,
                    recentlyTipped:true,
                    willBeVisible: local
    ...
    <div className="onlyContainer">
        <div className="box has-text-centered">
            <input type="text" autoFocus size={this.state.data.dePart.length + 3} value={this.state.inputFieldValue} onKeyPress={this.willHandleKeyPress}/>
        </div>
