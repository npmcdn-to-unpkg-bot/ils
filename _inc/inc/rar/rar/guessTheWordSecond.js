$(document).ready(function () {
    ...
    $('.firstToggle').click(function () {
        $(this).toggleClass("toggleClassActive");
        if (activeField == 0) {
            activeField = 1
        } else {
            activeField = 0
    ...
    // TOOLTIPS
    $('.entode').hover(function () {
        tooltip($(this), "for a single English word, it creates a set of English synonyms and their translation")
    })

    $('.reversoDetoen').hover(function () {
        tooltip($(this), "take German word and return example sentences")
    })

    $('.synonyms').hover(function () {
        tooltip($(this), "give me German word and I will get back plenty of its synonyms on both languages")
    })

    $('.synonymsPlus').hover(function () {
        tooltip($(this), "exchange single English word for pairs of Yandex synonyms and Yandex translations")
    })

    $('.bingImages').hover(function () {
        tooltip($(this), "use Bing as a source, no pagination")
    })

    $('.context').hover(function () {
        tooltip($(this), "gives you some Bing search results | not very well filtered")
    })

    $('.twitter').hover(function () {
        tooltip($(this), "expects German word and return well filtered results")
    })

    $('.welt').hover(function () {
        tooltip($(this), "expects single word and search for it in welt.de")
    })
    ...
    function initMainSecond() {
        processDataSecond.reset()
    }
    ...
    $('.upArrow').click(function () {
        ...
        $(window).scrollTop($('.scrollingSection' + scrollingCounter).offset().top);
    })

    $('.downArrow').click(function () {
        scrollingCounter++
        if (scrollingCounter > 4) {
            scrollingCounter = 4
        }
        $(window).scrollTop($('.scrollingSection' + scrollingCounter).offset().top);
    ...
    function resetInputs() {
        $("[id^=cre]").each(function () {
            $(this).val('');
        })
        $("[id^=spa]").each(function () {
            $(this).text('');
        })
    }
    ...
    function callbackSocketPattern(commandWord, postObject, callback) {
        socket = io.connect('//localhost:3700', {'forceNew': true});
    ...
    function inputSocketremove(commandWord) {
    ...
    function inputSocket(commandWord) {
        $('.' + commandWord).click(function () {
            ...
            socket.on(commandWord, function (data) {
                console.log(data)
                processDataSecond.setterCounter(0);
                processDataThird.setterCounter(0);
                processDataSecond.setter(data[0]);
                processDataThird.setter(data[1]);
                populateInputs()
                populateSpans()
            });
            $.ajax({
    ...
    function googleSocket(postObject) {
        ...
        $.ajax({
            type: "POST",
            url: "willBeSocks/googleTranslate",
            data: postObject
        })
            .done(function (msg) {
                notify()
    ...
    function beInsert(asValue, asValueSecond) {
        var indexFound;
        var indexFoundSecond;
        var flag = true;
        $('.willSave').each(function (index) {
            if ($(this).val() == '' && flag) {
                flag = false;
                indexFound = '#sav' + index
                indexFoundSecond = '#done' + index
                $(indexFound).val(asValue)
                $(indexFoundSecond).val(asValueSecond)
            }
        })
    }

    function beInsertSecond(asValue) {
        var indexFound;
        var keyFound;
        var flag = true;
        $('.willSave').each(function (index) {
            if ($(this).val() == '' && flag) {
                flag = false;
                keyFound = index - 1
                indexFound = '#done' + keyFound
                $(indexFound).val(asValue)
            }
        })
    }

    // next function with one more getter setter object
    ...
    function populateInputs() {
        var limit = 5;
        var start = processDataSecond.getterCounter()
        var end = processDataSecond.getterLimit()
        if (start + limit > end) {
            limit = end - start
        }
        for (i = 0; i < limit; i++) {
            var prev = processDataSecond.getterCounter()
            var ss = '#cre' + i;
            var value = processDataSecond.getterIndex(prev)
            $(ss).val(value)
            processDataSecond.setterCounter(start + i + 1)
        }
    }

    function populateSpans() {
        var limit = 5;
        var start = processDataThird.getterCounter()
        var end = processDataThird.getterLimit()
        if (start + limit > end) {
            limit = end - start
        }
        for (i = 0; i < limit; i++) {
            var prev = processDataThird.getterCounter()
            var ss = '#spa' + i;
            var value = processDataThird.getterIndex(prev)
            $(ss).text(value)
            processDataThird.setterCounter(start + i + 1)
        }
    }
    ...
    $('.btnSs').click(function () {
        ...
        var likeIdSecond = '#spa' + ss1[ss1.length - 1]
        var asValue = $(likeId).val()
        var asValueSecond = $(likeIdSecond).text()
        beInsert(asValue, asValueSecond)
    });

    $('.btnFirst').click(function () {
        var ss = $(this);
        var ss1 = ss.attr('id')
        var likeId = '#textarea' + ss1[ss1.length - 1]
        var asValue = $(likeId).val()
        console.log(asValue)
        beInsert(asValue)
    });
    ...
    // INITIALIZING REQUEST FUNCTIONS
    $('.translateSingle').click(function () {
        ...
        callbackSocketPattern('detoen', {query: likeValue}, function (data) {
    ...
    textareaSocket('welt')
    textareaSocket('context')
    textareaSocket('twitter')

    $('.submit').click(function () {
        var firstArray
        var secondArray

        $("[id^=sav]").each(function () {
            firstArray.push($(this).val());
        })

        $("[id^=done]").each(function () {
            secondArray.push($(this).val());
        })

        var finalObj = {};
        finalObj.mainWord = $('.mainWord').val()
        finalObj.synonymsArray = synonymsArray
        finalObj.imageUrl = imageUrl
        finalObj.german = firstArray
        finalObj.english = second

        $.ajax({
            type: "POST",
            url: "http://localhost:3002/post/guessTheWord",
            data: finalObj
        })
            .done(function (msg) {
                notify()
