$(document).ready(function () {
    ...
    //--||--||--||--||--||--||--||--||-|
    //SOCKET FUNCTIONS
    function textareaSocket(socketWord, postObject) {
        initMain()
    ...
    function inputSocket(getDataUrl, socketWord, postObject) {
        socket = io.connect('//localhost:3100', {'forceNew': true});
        socket.on('socketError', function (error) {
            console.log(error)
        });
    ...
    function reverso(postObject) {
        initMain()
        socket = io.connect('//localhost:3100', {'forceNew': true});
        socket.on('socketError', function (error) {
            console.log(error)
        });
        socket.on('reverso/detoenSecond', function (data) {
            console.log(data)
            processData.setterCounter(0);
            processData.setter(data);
            populateTextarea(data)
        ...
        $.ajax({
            type: "POST",
            url: "socks/reverso/detoenSecond",
            data: postObject
        })
            .done(function (msg) {
                setTimeout(function () {
    ...
    function detoenSocket(postObject) {
        socket = io.connect('//localhost:3100', {'forceNew': true});
        socket.on('socketError', function (error) {
            console.log(error)
        });
    ...
    $('.synonymsPlus').click(function () {
        var ss = whichField()
        inputSocket('socks/synonymsPlus', 'synonymsPlus', {query: ss})
    });
    ...
    $('.welt').click(function () {
        var ss = whichField()
        textareaSocket('welt', {query: ss})
    });

    $('.exactSearch').click(function () {
        var ss = whichField()
        textareaSocket('exactSearch', {query: ss})
    });
    ...
    $('.addTextareaSecond').click(function () {
        var ss = $(this);
        var ss1 = ss.attr('id')
        var likeId = '#textarea' + ss1[ss1.length - 1]
        var asValue = $(likeId).val();
    ...
    $('.translated').click(function () {
        var ss = $('#textareaTranslate').val();
        var ssThird = $('.mainWord').val();
        var ssSecond = $('#sav0').val()
        if (ssSecond !== '') {
            beInsert(ss)
        } else {
            beInsert(ssThird)
    ...
    $('.simpleTranslateLink').click(function () {
        var ss = $('.simpleTranslate').val();
        socket = io.connect('//localhost:3100', {'forceNew': true});
        socket.on('socketError', function (error) {
            console.log(error)
        });
        socket.on('detoen', function (data) {
            $('.simpleTranslated').text(data)
        });
        $.ajax({
            type: "POST",
            url: "socks/detoen",
